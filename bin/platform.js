 ...
 * @returns {string} Package name like "para-hyang-darwin-arm64"
 * @throws {Error} If libc cannot be detected on Linux
 */
export function getPlatformPackage({ platform, arch, libcFamily, packageBaseName = "para-hyang" }) {
  let suffix = "";
  if (platform === "linux") {
...
export function getPlatformPackageCandidates({ platform, arch, libcFamily, preferBaseline = false, packageBaseName = "para-hyang" }) {
...
/** @param {{ platform: string, arch: string, libcFamily?: string | null, packageBaseName?: string }} options */
function getBaselinePlatformPackage({ platform, arch, libcFamily, packageBaseName = "para-hyang" }) {
  if (arch !== "x64") {
    return null;
  }
 ...
 * @returns {string} Relative path like "para-hyang-darwin-arm64/bin/para-hyang"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/para-hyang${ext}`;
}

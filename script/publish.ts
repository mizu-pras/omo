...
console.log("=== Publishing para-hyang (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `para-hyang-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  const team = ["actions-user", "github-actions[bot]", "mizu-pras"]

  try {
    const compare =
      await $`gh api "/repos/mizu-pras/omo/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/para-hyang.schema.json`
  for (const platform of PLATFORM_PACKAGES) {

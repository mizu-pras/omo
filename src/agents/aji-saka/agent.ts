...
function buildDynamicOrchestratorPrompt(ctx?: OrchestratorContext): string {
  ...
  const agentIdentity = buildAgentIdentitySection(
    "Aji Saka",
    "Master Orchestrator agent from ParaHyang that coordinates specialized agents to complete todo lists",
  )
...
export function createAtlasAgent(ctx: OrchestratorContext): AgentConfig {
  const baseConfig = {
    description:
      "Orchestrates work via task() to complete ALL tasks in a todo list until fully done. (Aji Saka - ParaHyang)",
    mode: MODE,
...
export const atlasPromptMetadata: AgentPromptMetadata = {
  ...
  useWhen: [
    "User provides a todo list path (.ismaya/plans/{name}.md)",

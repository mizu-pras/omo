...
describe("Atlas prompts plan path consistency", () => {
  test("default variant should use .ismaya/plans/{plan-name}.md path", () => {
    // given
    const prompt = ATLAS_SYSTEM_PROMPT

    // when / then
    expect(prompt).toContain(".ismaya/plans/{plan-name}.md")
    expect(prompt).not.toContain(".ismaya/tasks/{plan-name}.yaml")
    expect(prompt).not.toContain(".ismaya/tasks/")
  })

  test("gpt variant should use .ismaya/plans/{plan-name}.md path", () => {
    // given
    const prompt = ATLAS_GPT_SYSTEM_PROMPT
  ...
  test("gemini variant should use .ismaya/plans/{plan-name}.md path", () => {
    // given
    const prompt = ATLAS_GEMINI_SYSTEM_PROMPT
  ...
  test("all variants should read plan file after verification", () => {
    ...
    // when / then
    for (const prompt of prompts) {
      expect(prompt).toMatch(/read[\s\S]*?\.ismaya\/plans\//)
    }
  })

...
describe("ATLAS prompt checkbox enforcement", () => {
  describe("default prompt", () => {
    ...
    test("boundaries should include exception for editing .ismaya/plans/*.md checkboxes", () => {
      // given
      const prompt = ATLAS_SYSTEM_PROMPT
      const lowerPrompt = prompt.toLowerCase()

      // when / then
      expect(lowerPrompt).toMatch(/\.ismaya\/plans\/\*\.md/)
    ...
    test("default prompt should NOT reference .ismaya/tasks/", () => {
      // given
      const prompt = ATLAS_SYSTEM_PROMPT

      // when / then
      expect(prompt).not.toMatch(/\.ismaya\/tasks\//)
    })
  })

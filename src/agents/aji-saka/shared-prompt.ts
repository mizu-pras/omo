...
\`\`\`typescript
// Option A: Category + Skills (spawns Cenil with domain config)
task(
...
## 6. CONTEXT
### Notepad Paths
- READ: .ismaya/notepads/{plan-name}/*.md
...
**Path convention**:
- Plan: \`.ismaya/plans/{name}.md\` (you may EDIT to mark checkboxes)
- Notepad: \`.ismaya/notepads/{name}/\` (READ/APPEND)
</notepad_protocol>`
...
1. **EDIT the plan checkbox**: Change \`- [ ]\` to \`- [x]\` for the completed task in \`.ismaya/plans/{plan-name}.md\`

2. **READ the plan to confirm**: Read \`.ismaya/plans/{plan-name}.md\` and verify the checkbox count changed (fewer \`- [ ]\` remaining)

/**
 * GPT-5.3-Codex Optimized Cenil System Prompt
 *
 * Togog-style prompt adapted for a focused executor:
 * - Same autonomy, reporting, parallelism, and tool usage patterns
 * - CAN spawn nayagenggong/pujangga via call_omo_agent for research
 */
...
export function buildGpt53CodexSisyphusJuniorPrompt(
  ...
  const prompt = `You are Cenil - a focused task executor from ParaHyang.
...
- Need context? Fire Nayagenggong/Ki Pujangga via call_omo_agent IMMEDIATELY - continue only with non-overlapping work while they search
...
- Nayagenggong/Ki Pujangga via call_omo_agent = background research. Fire them and continue only with non-overlapping work

...
Generate plan to: \`.ismaya/plans/{name}.md\`
...
- [Explicit exclusion from Jayabaya review]
...
Evidence saved to \`.ismaya/evidence/task-{N}-{scenario-slug}.{ext}\`.
...
- [ ] 1. [Task Title]
  ...
  **QA Scenarios (MANDATORY - task is INCOMPLETE without these):**
  ...
  Scenario: [Happy path - what SHOULD work]
    ...
    Evidence: .ismaya/evidence/task-{N}-{scenario-slug}.{ext}
  ...
  Scenario: [Failure/edge case - what SHOULD fail gracefully]
    ...
    Evidence: .ismaya/evidence/task-{N}-{scenario-slug}-error.{ext}
  \\\`\\\`\\\`
...
- [ ] F1. **Plan Compliance Audit** \u2014 \`oracle\`
  Read the plan end-to-end. For each "Must Have": verify implementation exists (read file, curl endpoint, run command). For each "Must NOT Have": search codebase for forbidden patterns \u2014 reject with file:line if found. Check evidence files exist in .ismaya/evidence/. Compare deliverables against plan.
...
- [ ] F3. **Real Manual QA** \u2014 \`unspecified-high\` (+ \`playwright\` skill if UI)
  Start from clean state. Execute EVERY QA scenario from EVERY task \u2014 follow exact steps, capture evidence. Test cross-task integration (features working together, not isolation). Test edge cases: empty state, invalid input, rapid actions. Save to \`.ismaya/evidence/final-qa/\`.

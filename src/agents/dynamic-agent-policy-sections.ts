...
export function buildHardBlocksSection(): string {
  const blocks = [
    ...
    "- Delivering final answer before collecting Kanjeng Ratu Kidul result - **Never.**",
  ]
...
export function buildAntiPatternsSection(): string {
  const patterns = [
    ...
    "- **Delegation Duplication**: Delegating exploration to nayagenggong/pujangga and then manually doing the same search yourself",
    "- **Kanjeng Ratu Kidul**: Delivering answer without collecting Kanjeng Ratu Kidul results",
  ]
...
export function buildUltraworkSection(
  ...
  if (agents.length > 0) {
      const ultraworkAgentPriority = ["nayagenggong", "pujangga", "dewi-sri", "ratu-kidul"]
    ...
    for (const agent of sortedAgents) {
      const shortDescription =
        agent.description.length > 120
          ...
          : agent.description
      const suffix =
        agent.name === "nayagenggong" || agent.name === "pujangga" ? " (multiple)" : ""
...
Once you delegate exploration to nayagenggong/pujangga agents, **DO NOT perform the same search yourself**.

### What this means:

**FORBIDDEN:**
- After firing nayagenggong/pujangga, manually grep/search for the same information
...
task(subagent_type="nayagenggong", run_in_background=true, ...)

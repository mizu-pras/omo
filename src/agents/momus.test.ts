...
import { MOMUS_SYSTEM_PROMPT } from "./sabdapalon"
...
describe("MOMUS_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .ismaya/plans/ and ending in .md", () => {
    // given
    const prompt = MOMUS_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".ismaya/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .ismaya/plans/plan.md"

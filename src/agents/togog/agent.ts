...
function buildDynamicHephaestusPrompt(ctx?: HephaestusContext): string {
  ...
  const agentIdentity = buildAgentIdentitySection(
    "Togog",
    "Autonomous deep worker for software engineering from ParaHyang",
  );
...
export function createHephaestusAgent(
  ...
  return {
    description:
      "Autonomous Deep Worker - goal-oriented execution with GPT Codex. Explores thoroughly before acting, uses Nayagenggong/Ki Pujangga for comprehensive context, completes tasks end-to-end. Inspired by AmpCode deep mode. (Togog - ParaHyang)",
    mode: MODE,
    model,
    maxTokens: 32000,
    prompt,
...
export const hephaestusPromptMetadata: AgentPromptMetadata = {
  ...
  avoidWhen: [
    ...
    "When orchestration across multiple agents is needed (use Aji Saka)",
  ],

...
describe("detectCurrentConfig - single package detection", () => {
  ...
  it("detects both legacy and canonical plugin entries", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang", "para-hyang@3.11.0"] }, null, 2) + "\n", "utf-8")
  ...
  it("returns false when plugin not present with similar name", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang-extra"] }, null, 2) + "\n", "utf-8")
  ...
  it("detects OpenCode Go from the existing omo config", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang"] }, null, 2) + "\n", "utf-8")
...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  it("writes canonical plugin entry for new installs", async () => {
    ...
    expect(savedConfig.plugin).toEqual(["para-hyang"])
  })
  ...
  it("updates a version-pinned legacy entry to the requested version", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion")
    getPluginNameWithVersionSpy.mockResolvedValue("para-hyang@3.16.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang@3.15.0"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["para-hyang@3.16.0"])
    getPluginNameWithVersionSpy.mockRestore()
  })

  it("removes stale legacy entry when canonical and legacy entries both exist", async () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang", "oh-my-opencode"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["oh-my-opencode", "para-hyang"])
  })

  it("preserves a canonical entry when the same version is re-installed", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion")
    getPluginNameWithVersionSpy.mockResolvedValue("para-hyang@3.10.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang@3.10.0"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["para-hyang@3.10.0"])
    getPluginNameWithVersionSpy.mockRestore()
  })
  ...
  it("blocks a downgrade for a version-pinned canonical entry", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion")
    getPluginNameWithVersionSpy.mockResolvedValue("para-hyang@3.15.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["para-hyang@3.16.0"] }, null, 2) + "\n", "utf-8")

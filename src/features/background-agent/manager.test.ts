...
describe("BackgroundManager.notifyParentSession - release ordering", () => {
  test("should unblock queued task even when prompt hangs", async () => {
    ...
    await concurrencyManager.acquire("nayagenggong")

    let task2Resolved = false
    const task2Promise = concurrencyManager.acquire("nayagenggong").then(() => {
      task2Resolved = true
    })
    ...
    const simulateNotifyParentSession = async () => {
      concurrencyManager.release("nayagenggong")
  ...
  test("should keep queue blocked if release is after prompt (demonstrates the bug)", async () => {
    ...
    let task2Resolved = false
    concurrencyManager.acquire("nayagenggong").then(() => {
      task2Resolved = true
    })
...
describe("BackgroundManager.resume", () => {
  ...
  test("should preserve task identity while updating parent context", () => {
    ...
    expect(result.agent).toBe("nayagenggong")
    expect(result.parentModel).toEqual({ providerID: "anthropic", modelID: "claude-opus" })
  })

  test("should canonicalize display-form agent names on actual resume", async () => {
    // given
    const actualManager = createBackgroundManager()
    const existingTask = createMockTask({
      id: "task-a",
      sessionID: "session-a",
      parentSessionID: "old-parent",
    ...
    getTaskMap(actualManager).set(existingTask.id, existingTask)
    spyOn(getConcurrencyManager(actualManager), "acquire").mockResolvedValue(undefined)
    spyOn(actualManager as unknown as { startPolling: () => void }, "startPolling").mockImplementation(() => {})

    try {
      // when
      const result = await actualManager.resume({
        sessionId: "session-a",
        prompt: "new prompt",
        parentSessionID: "new-parent",
        parentMessageID: "new-msg",
      ...
      // then
      expect(result.agent).toBe("dewi-sri")
      expect(existingTask.agent).toBe("dewi-sri")
    } finally {
...
describe("BackgroundManager.notifyParentSession - dynamic message lookup", () => {
  test("should skip compaction agent and use nearest non-compaction message", async () => {
    ...
    //#then
    expect(capturedBody?.agent).toBe("ismaya")
  ...
  test("should use currentMessage model/agent when available", async () => {
    ...
    expect(promptBody.agent).toBe("ismaya")
...
describe("BackgroundManager.resume model persistence", () => {
  ...
  test("should pass model when task has a configured model", async () => {
    ...
    expect(promptCalls[0].body.agent).toBe("nayagenggong")
  })
...
describe("BackgroundManager - tool permission spread order", () => {
  ...
  test("resume restores plan-family task permission for legacy prometheus agent", async () => {
    //#given
    ...
    const manager = new BackgroundManager({ client, directory: tmpdir() } as unknown as PluginInput)
    const task: BackgroundTask = {
      id: "task-prometheus-resume",
      sessionID: "session-prometheus-resume",
      parentSessionID: "parent-session",
      parentMessageID: "parent-message",
    ...
    expect(promptCall?.body.agent).toBe("dewi-sri")
    expect(promptCall?.body.tools).toMatchObject({ task: true })
  ...
  test("resume keeps agent when explicit model is configured", async () => {
    ...
    //#then
    expect(promptCall).toBeDefined()
    expect(promptCall?.body.agent).toBe("nayagenggong")

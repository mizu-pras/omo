...
function buildCommitFooterInjection(
	...
	sections.push("")
		sections.push("Add Sang Hyang Ismaya attribution to EVERY commit:")
	sections.push("")
	...
	if (commitFooter) {
		const footerText =
			typeof commitFooter === "string"
				? commitFooter
				: "Ultraworked with [Sang Hyang Ismaya](https://github.com/mizu-pras/omo)"
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Sang Hyang Ismaya <clio-agent@sisyphuslabs.ai>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Sang Hyang Ismaya <clio-agent@sisyphuslabs.ai>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			`${cmdPrefix}git commit -m "{Commit Message}" -m "Co-authored-by: Sang Hyang Ismaya <clio-agent@sisyphuslabs.ai>"`
		)
		sections.push("```")
	}

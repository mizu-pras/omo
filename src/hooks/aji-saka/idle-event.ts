...
import type { AjiSakaHookOptions, SessionState } from "./types"
...
function hasRunningBackgroundTasks(sessionID: string, options?: AjiSakaHookOptions): boolean {
  ...
  try {
    ...
    if (!canContinueSession) {
      log(`[${HOOK_NAME}] Skipped: tracked descendant agent does not match boulder agent`, {
        sessionID: input.sessionID,
        requiredAgent: currentBoulder.agent ?? "aji-saka",
      })
      return
    }
...
export async function handleAtlasSessionIdle(input: {
  ...
  if (!canContinueSession) {
    log(`[${HOOK_NAME}] Skipped: tracked descendant agent does not match boulder agent`, {
      sessionID,
      requiredAgent: boulderState.agent ?? "aji-saka",
    })
    return
  }
...
async function canContinueTrackedBoulderSession(input: {
  ...
  const requiredAgentKey = getAgentConfigKey(input.requiredAgent ?? "aji-saka")
  const sessionAgentKey = getAgentConfigKey(sessionAgent)
  return sessionAgentKey === requiredAgentKey
    || (requiredAgentKey === getAgentConfigKey("aji-saka") && sessionAgentKey === getAgentConfigKey("ismaya"))
}

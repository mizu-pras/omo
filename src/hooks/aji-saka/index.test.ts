...
const { createAjiSakaHook: createAtlasHook } = await import("./index")
...
describe("atlas hook", () => {
  ...
  beforeEach(() => {
    ...
    SISYPHUS_DIR = join(TEST_DIR, ".ismaya")
  ...
  describe("tool.execute.after handler", () => {
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should append standalone verification when no boulder state but caller is Atlas", async () => {
       ...
       setupMessageStorage(sessionID, "aji-saka")
...
session_id: ses_subagent_abc
      ...
      expect(output.metadata.agent).toBe("cenil")
...
session_id: ses_auth_flow_123
     ...
     const updatedState = readBoulderState(TEST_DIR)
      ...
      expect(updatedState?.task_sessions?.["todo:1"]?.agent).toBe("cenil")
...
session_id: ses_untrusted_999
    ...
    describe("completion gate output ordering", () => {
      ...
      beforeEach(() => {
        setupMessageStorage(COMPLETION_GATE_SESSION, "aji-saka")
      })
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "aji-saka")
       })
      ...
      test("should append delegation reminder when orchestrator writes outside .ismaya/", async () => {
        // given
        const hook = createAtlasHook(createMockPluginInput())
        const output = {
          title: "Write",
      ...
      test("should append delegation reminder when orchestrator edits outside .ismaya/", async () => {
        // given
        const hook = createAtlasHook(createMockPluginInput())
        const output = {
          title: "Edit",
      ...
      test("should NOT append reminder when orchestrator writes inside .ismaya/", async () => {
        // given
        const hook = createAtlasHook(createMockPluginInput())
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.ismaya/plans/work-plan.md" },
        }
      ...
      test("should NOT append reminder when non-orchestrator writes outside .ismaya/", async () => {
        // given
        const nonOrchestratorSession = "non-orchestrator-session"
        setupMessageStorage(nonOrchestratorSession, "cenil")
      ...
      describe("cross-platform path validation (Windows support)", () => {
        test("should NOT append reminder when orchestrator writes inside .ismaya\\ (Windows backslash)", async () => {
          // given
          const hook = createAtlasHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: ".ismaya\\plans\\work-plan.md" },
          }
        ...
        test("should NOT append reminder when orchestrator writes inside .ismaya with mixed separators", async () => {
          // given
          const hook = createAtlasHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
        ...
        test("should NOT append reminder for absolute Windows path inside .ismaya\\", async () => {
          // given
          const hook = createAtlasHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: "C:\\Users\\test\\project\\.ismaya\\plans\\x.md" },
          }
        ...
        test("should append reminder for Windows path outside .ismaya\\", async () => {
          // given
          const hook = createAtlasHook(createMockPluginInput())
          const output = {
            title: "Write",
  ...
  describe("session.idle handler (boulder continuation)", () => {
    ...
    async function flushMicrotasks(): Promise<void> {
     ...
     beforeEach(() => {
       ...
       registerAgentName("ismaya")
        ...
        setupMessageStorage(MAIN_SESSION_ID, "aji-saka")
      })
    ...
    test("should not append lineage-only subagent session during idle without explicit boulder tracking", async () => {
      ...
      updateSessionAgent(subagentSessionID, "aji-saka")
    ...
    test("should inject when registered boulder session has incomplete tasks even if last agent differs", async () => {
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "togog")
...
`)
    ...
    test("should inject when last agent is sisyphus and boulder targets atlas explicitly", async () => {
       ...
       cleanupMessageStorage(MAIN_SESSION_ID)
       setupMessageStorage(MAIN_SESSION_ID, "ismaya")
    ...
    test("should inject when registered atlas boulder session last agent does not match", async () => {
     ...
     test("should inject when last agent matches boulder agent even if non-Atlas", async () => {
       ...
       // then - should call prompt for canonical config key
       expect(mockInput._promptMock).toHaveBeenCalled()
       const callArgs = mockInput._promptMock.mock.calls[0][0]
       expect(callArgs.body.agent).toBe("ismaya")
      })

    test("should canonicalize display-name agent in continuation prompt", async () => {
      ...
      expect(mockInput._promptMock).toHaveBeenCalled()
      const callArgs = mockInput._promptMock.mock.calls[0][0]
      expect(callArgs.body.agent).toBe("aji-saka")
    })
    ...
    test("should inject when session agent was updated to atlas by start-work even if message storage agent differs", async () => {
      ...
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "togog")
      updateSessionAgent(MAIN_SESSION_ID, "aji-saka")

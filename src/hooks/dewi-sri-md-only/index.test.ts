...
const { createDewiSriMdOnlyHook: createPrometheusMdOnlyHook } = await import("./index")
const { MESSAGE_STORAGE } = await import("../../features/hook-message-injector")

describe("dewi-sri-md-only", () => {
  ...
  describe("agent name matching", () => {
    test("should enforce md-only restriction for exact prometheus agent name", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
      ...
      ).rejects.toThrow("File operations restricted to .ismaya/*.md plan files only")
    })

    test("should enforce md-only restriction for canonical dewi-sri agent name", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
    ...
    test("should enforce md-only restriction for uppercase DEWI-SRI", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "DEWI-SRI")
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
    ...
    test("should enforce md-only restriction for uppercase DEWI SRI config-style token", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
    ...
    test("should not enforce restriction for non-Prometheus agent", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "ismaya")
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
        sessionID: TEST_SESSION_ID,
   ...
   describe("with Prometheus agent in message storage", () => {
    ...
    test("should allow Prometheus to write .md files inside .ismaya/", async () => {
      // given
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      const output = {
        args: { filePath: "/tmp/test/.ismaya/plans/work-plan.md" },
      }
    ...
    test("should inject workflow reminder when Prometheus writes to .ismaya/plans/", async () => {
      // given
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      // then
      expect(output.message).toContain("DEWI SRI MANDATORY WORKFLOW REMINDER")
      expect(output.message).toContain("INTERVIEW")
      expect(output.message).toContain("JAYABAYA CONSULTATION")
      expect(output.message).toContain("SABDAPALON REVIEW")
    })

    test("should NOT inject workflow reminder for .ismaya/drafts/", async () => {
      // given
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      const output: { args: Record<string, unknown>; message?: string } = {
        args: { filePath: "/tmp/test/.ismaya/drafts/notes.md" },
      }
    ...
    test("should block Prometheus from writing .md files outside .ismaya/", async () => {
      // given
      const hook = createPrometheusMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
  ...
  describe("boulder state priority over message files (fixes #927)", () => {
    ...
    const BOULDER_FILE = join(BOULDER_DIR, ".ismaya", "boulder.json")

    beforeEach(() => {
      mkdirSync(join(BOULDER_DIR, ".ismaya"), { recursive: true })
    })
    ...
    test("should use prometheus from boulder state when set", async () => {
      ...
      setupMessageStorage(TEST_SESSION_ID, "aji-saka", { useSessionAgent: false })
  ...
  describe("cross-platform path validation", () => {
    beforeEach(() => {
     ...
     test("should allow Windows-style backslash paths under .ismaya/", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
       const hook = createPrometheusMdOnlyHook(createMockPluginInput())
       const input = {
       ...
       const output = {
         args: { filePath: ".ismaya\\plans\\work-plan.md" },
       }
     ...
     test("should allow mixed separator paths under .ismaya/", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
       const hook = createPrometheusMdOnlyHook(createMockPluginInput())
       const input = {
     ...
     test("should block paths outside workspace root even if containing .ismaya", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
       const hook = createPrometheusMdOnlyHook(createMockPluginInput())
       const input = {
       ...
       const output = {
         args: { filePath: "/other/project/.ismaya/plans/x.md" },
       }
     ...
     test("should allow nested .ismaya directories (ctx.directory may be parent)", async () => {
       ...
       const output = {
         args: { filePath: "src/.ismaya/plans/x.md" },
       }

       // when / #then - should allow because .ismaya is in path
       await expect(
         hook["tool.execute.before"](input, output)
       ).resolves.toBeUndefined()
     })
     ...
     test("should block path traversal attempts", async () => {
       ...
       const output = {
         args: { filePath: ".ismaya/../secrets.md" },
       }
     ...
     test("should allow case-insensitive .ISMAYA directory", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
       const hook = createPrometheusMdOnlyHook(createMockPluginInput())
       const input = {
     ...
     test("should allow nested project path with .ismaya (Windows real-world case)", async () => {
       // given - simulates when ctx.directory is parent of actual project
       // User reported: xauusd-dxy-plan\.ismaya\drafts\supabase-email-templates.md
       setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
       const hook = createPrometheusMdOnlyHook(createMockPluginInput())
       const input = {
         tool: "Write",
       ...
       const output = {
         args: { filePath: "xauusd-dxy-plan\\.ismaya\\drafts\\supabase-email-templates.md" },
       }
     ...
     test("should allow nested project path with mixed separators", async () => {
       ...
       const output = {
         args: { filePath: "my-project/.ismaya\\plans/task.md" },
       }
     ...
     test("should block nested project path without .ismaya", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "dewi-sri")
       const hook = createPrometheusMdOnlyHook(createMockPluginInput())
       const input = {

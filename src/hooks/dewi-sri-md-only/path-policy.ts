...
/**
 * Cross-platform path validator for Dewi Sri file writes.
 * Uses path.resolve/relative instead of string matching to handle:
 * - Windows backslashes (e.g., .ismaya\\plans\\x.md)
 * - Mixed separators (e.g., .ismaya\\plans/x.md)
 * - Case-insensitive directory/extension matching
 * - Workspace confinement (blocks paths outside root or via traversal)
 * - Nested project paths (e.g., parent/.ismaya/... when ctx.directory is parent)
 */
export function isAllowedFile(filePath: string, workspaceRoot: string): boolean {
  ...
  // 4. Check if .ismaya/ or .ismaya\ exists anywhere in the path (case-insensitive)
  // This handles both direct paths (.ismaya/x.md) and nested paths (project/.ismaya/x.md)
  if (!/\.ismaya[/\\]/i.test(rel)) {
    return false
  }

...
/ralph-loop → startLoop(sessionID, prompt, options)
  → loopState.startLoop() → persists state to .ismaya/ralph-loop.local.md
...
| `storage.ts` | Read/write `.ismaya/ralph-loop.local.md` state file |
...
```
.ismaya/ralph-loop.local.md  (gitignored)

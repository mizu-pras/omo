...
describe("migrateLegacyConfigFile", () => {
  ...
  describe("#given oh-my-opencode.jsonc exists but para-hyang.jsonc does not", () => {
    describe("#when migrating the config file", () => {
      it("#then writes para-hyang.jsonc and renames the legacy file to a backup", () => {
        ...
        expect(existsSync(join(testDir, "para-hyang.jsonc"))).toBe(true)
        expect(existsSync(legacyPath)).toBe(false)
        expect(existsSync(backupPath)).toBe(true)
        expect(readFileSync(join(testDir, "para-hyang.jsonc"), "utf-8")).toBe('{ "agents": {} }')
        expect(readFileSync(backupPath, "utf-8")).toBe('{ "agents": {} }')
      })
    })
  })

  describe("#given oh-my-opencode.json exists but para-hyang.json does not", () => {
    describe("#when migrating the config file", () => {
      it("#then copies to para-hyang.json", () => {
        ...
        expect(result).toBe(true)
        expect(existsSync(join(testDir, "para-hyang.json"))).toBe(true)
      })
    })
  })

  describe("#given para-hyang.jsonc already exists", () => {
    describe("#when attempting migration", () => {
      it("#then archives the legacy alias and does not overwrite the canonical file", () => {
        const legacyPath = join(testDir, "oh-my-opencode.jsonc")
        const backupPath = `${legacyPath}.bak`
        const canonicalPath = join(testDir, "para-hyang.jsonc")
  ...
  describe("#given oh-my-openagent.jsonc exists but para-hyang.jsonc does not", () => {
    describe("#when migrating the config file", () => {
      it("#then writes para-hyang.jsonc and archives oh-my-openagent.jsonc", () => {
        const legacyPath = join(testDir, "oh-my-openagent.jsonc")
        const backupPath = `${legacyPath}.bak`
        writeFileSync(legacyPath, '{ "agents": { "ismaya": {} } }')

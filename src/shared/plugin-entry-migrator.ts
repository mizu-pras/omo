...
export function isLegacyEntry(entry: string): boolean {
  return entry === LEGACY_PLUGIN_NAME || entry.startsWith(`${LEGACY_PLUGIN_NAME}@`)
    || entry === SECONDARY_LEGACY_PLUGIN_NAME || entry.startsWith(`${SECONDARY_LEGACY_PLUGIN_NAME}@`)
}
...
export function toCanonicalEntry(entry: string): string {
  ...
  if (entry === SECONDARY_LEGACY_PLUGIN_NAME) {
    return PLUGIN_NAME
  }

  if (entry.startsWith(`${SECONDARY_LEGACY_PLUGIN_NAME}@`)) {
    return `${PLUGIN_NAME}${entry.slice(SECONDARY_LEGACY_PLUGIN_NAME.length)}`
  }
